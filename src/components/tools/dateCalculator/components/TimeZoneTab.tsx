...
/**
 * TimeZoneTab.tsx
 * Advanced world clock interface with modern timezone cards, search functionality, and interactive features
 */
...
import { Globe, Search, Star, Copy, Clock, MapPin, Heart } from "lucide-react";
...
export const TimeZoneTab = () => {
  ...
  // Extended timezone data with more cities
  const timeZones = [
  ...
  const toggleFavorite = (timezoneName: string) => {
    ...
    toast({
      title: favorites.includes(timezoneName) ? "Retiré des favoris" : "Ajouté aux favoris",
      description: `${timeZones.find(tz => tz.name === timezoneName)?.city} ${favorites.includes(timezoneName) ? 'retiré des' : 'ajouté aux'} favoris.`,
    });
  };
  ...
  // Add/remove timezone from comparison
  const toggleComparison = (timezoneName: string) => {
    if (selectedForComparison.includes(timezoneName)) {
      setSelectedForComparison(prev => prev.filter(tz => tz !== timezoneName));
    } else if (selectedForComparison.length < 4) {
      setSelectedForComparison(prev => [...prev, timezoneName]);
    } else {
      toast({
                   ...
                   <div className="flex gap-1">
                     {compareMode && (
                       <Button
                         variant="ghost"
                         size="sm"
                         onClick={() => toggleComparison(tz.name)}
